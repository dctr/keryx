...
2. Validate the task body against `schemas/action-item.v1.schema.json`: write the body to a temp file (e.g. `/tmp/keryx-card.$$.json`) and run `hermes keryx validate-card /tmp/keryx-card.$$.json`. This includes the `schema == "keryx.action_item.v1"` const check. Reject malformed cards by blocking with the validation reason. Use `./bin/opsctl validate-card <file>` from the repo root only as the direct fallback.
3. Read task comments and find the latest valid trusted execution decision comment. For each candidate, write the comment JSON to a temp file (e.g. `/tmp/keryx-decision.$$.json`) and validate it against `schemas/execution-decision.v1.schema.json` with `hermes keryx validate-decision /tmp/keryx-decision.$$.json` (or `./bin/opsctl validate-decision <file>` as fallback). This includes the `schema == "keryx.execution_decision.v1"` const check. Select the latest comment that validates.
...
5. When field semantics are uncertain, consult `hermes keryx schema action-item` and `hermes keryx schema execution-decision` (or `./bin/opsctl schema ...`) for the canonical contract.
...
7. Execute only the selected option's `execution_prompt`, plus the user's trusted `user_feedback` from the decision comment.
