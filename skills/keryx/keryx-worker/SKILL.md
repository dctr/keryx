...
- If the feedback is generic or can be generalised, create a separate blocked card on board `keryx` suggesting an update to the relevant `keryx:keryx-collector-$SOURCE` skill's automations list, or the explicitly named external source skill if it is not shipped by the Keryx plugin. Do not modify the skill directly from this worker run.
